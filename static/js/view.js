...
export function bookingPage(data) {
  ...
  if (result === null) {
    ...
    const newContent = new CreateElement(
      "div",
      "content margin",
      "目前沒有任何待預定的行程"
    ).e;
...
export function getOrderList(data) {
  Object.values(data).forEach((element) => {
    ...
    const date = new CreateElement("div", "order-content date", element.date).e;
    const time = new CreateElement("div", "order-content time", element.time).e;
